...
class GAN(object):
    ...
    def __init__(self, num_of_corpus=3, batch_size=50, n_noise=100, seed=1):
    ...
    def post_process(self, data, n_words=20, n_docs=50, categories = ['world', 'us', 'politics'], stemming_dict='data/stemming_dict.pkl.gz', stopwords='data/stopwords.pkl.gz',
        filename='result/gan/test_data.txt'):
    ...
    def test_gan(self, n_epochs=500, n_post_epochs=500, save=True, output='result/gan/gan-1.out'):
        ...
        if save:
            ...
            pickle.dump(np.vstack([test_data() for i in range(10000//self.batch_size)]), f, protocol=2)
            f.close()
