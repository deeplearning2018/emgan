...
categories = ['earn', 'acq', 'crude', 'trade', 'money-fx', 'interest', 'money-supply', 'ship']
data = ['train', 'test']
for d in data:
    w = dict()
    for category in categories:
        w[category] = open('data/cleaned/cleaned_%s_%s.txt' %(category, d), 'w')
    for line in open('data/r52-%s-stemmed.txt' %d, 'r'):
        columns = line.strip().split('\t')
        type = columns[0]
        if len(columns) == 2 and columns[0] in categories and columns[1] != '':
...
vocab = [vocab[i] for i in np.argsort(count_of_vocab)[::-1]]
indices_of_vocab = dict({vocab[i] : i for i in range(len(vocab))})
...
DF = np.zeros((8, len(vocab)), dtype='float32')
N = [0]*8
for i in range(8):
...
idf = np.zeros((9, len(vocab)), dtype='float32')
for i in range(8):
    idf[i,:] = np.log(N[i]/np.maximum(1., DF[i,:]))

idf[8,:] = np.log(sum(N)/np.maximum(1., DF.sum(axis=0)))
np.save('idf.npy', idf)
