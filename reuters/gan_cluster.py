...
class GAN(object):
    ...
    def __init__(self, num_of_corpus=8, batch_size=50, seed=8888):
        ...
        print('n_train: %d' %(sum([self.X_train[i].get_value(borrow=True).shape[0] for i in range(self.num_of_corpus)])))
        print('n_valid: %d' %(sum([self.X_valid[i].get_value(borrow=True).shape[0] for i in range(self.num_of_corpus)])))
        print('n_test: %d' %(sum([self.X_test[i].get_value(borrow=True).shape[0] for i in range(self.num_of_corpus)])))
        sys.exit(1)
    ...
    def test_gan(self, n_preepochs=100, n_epochs=500, output='gan-8888.out'):
        ...
        for epoch in range(n_preepochs):
            lr = 0.01 + 0.09*epoch/n_preepochs
