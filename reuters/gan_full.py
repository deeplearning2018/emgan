...
class GAN(object):
    ...
    def __init__(self, num_of_corpus=8, batch_size=50, n_noise=100, seed=8888):
    ...
    def post_process(self, data, n_words=20, filename='result/gan/test_data.txt'):
        ...
        for i in range(self.num_of_corpus):
            ...
            for j in range(category.shape[0]):
                ind = np.argsort(category[j,:])[::-1][:n_words]
                for k in ind:
                    os.write(vocab[k] + ' ')
                os.write('\n')
    ...
    def test_gan(self, n_epochs=500, n_post_epochs=500, save=False, output='result/gan/gan-8888.out'):
